...
if __name__ == '__main__':
    ...
    filename = 'card_sweep4'  # Location for our HDF5 file
    sweep_length = 20
    sweep = []
    ...
    if os.access(filename + '.h5', os.F_OK):  # ...retrieve the Waveforms from file.
        ...
        for i in range(sweep_length+1):
            ...
            sweep.append(waveform.Superposition(f, phases=phases[:len(freq_A)]))

        ## Compute all the Sample Points ##
        for i in range(len(sweep)):
            sweep[i].compute_waveform(filename, 'sweep'+str([i]))
