        ...
        if path != self.last_created:
            ...
            if time_diff > 0.8:
                ...
                if 0 < atom_count:
                    ...
                    boundary = 30 #empty_list[int(num_empty / 2)]
...
if __name__ == '__main__':
    ...
    llSegment = 2 * num_tweezers - 2 # associated data memory segment is static waveform, 16lambda
    llLoop = 1  # pattern repeated once
    ...
    lStep = 1000
    llSegment = len(wf_list_drop)-2 # jump to 5lambda static
    llLoop = 1
    llNext = 1001  # this is a dummy step
    llCondition = SPCSEQ_ENDLOOPONTRIG  # unconditionally leave current step
    llValue = (llCondition << 32) | (llLoop << 32) | (llNext << 16) | (llSegment)
    spcm_dwSetParam_i64(hCard, SPC_SEQMODE_STEPMEM0 + lStep, int64(llValue))
