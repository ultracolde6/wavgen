...
'''
FOUR LAMBDA SPACING SWEEP GENERATION'''
...
if __name__ == '__main__':
    ...
    spacing_Lambda_A = 5.5
    stagger_Lambda_A = 0
    shift_Lambda_A = 1/4


    for stagger_Lambda_A in np.arange(-1/4,1/4+0.00000001,1/32):
        ...
        freq_A = [startfreq_A + j * spacing_A + stagger_A * (-1) ** (j + 1) for j in range(ntraps)]
        ...
        freq_B = [startfreq_B + j * spacing_B + stagger_B * (-1) ** (j + 1) for j in range(ntraps)]
        ...
        if os.access(filename, os.F_OK):  # ...retrieve the Waveforms from file.
            ...
            AB = Sweep_sequence(A, B, sweep_time=sweeptime_ms, ramp='cosine', segment = False)
