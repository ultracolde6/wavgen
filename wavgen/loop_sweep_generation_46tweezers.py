...
if __name__ == '__main__':
    ...
    ntraps = 47  # this is the num of tweezers we want, plus 1
    # keep_num = 30
    # center_freq = 100.3E6
    ...
    for sweep_mode in ["cosine"]:
        for sweep_time in [0.16]:
            ...
            # max_list = []
            for sweep_num in np.arange(ntraps-2)+1:
                name_temp = 'sweep46tweezers_{}R.h5'.format(sweep_num)
                ...
                if os.access(filename, os.F_OK):  # ...retrieve the Waveforms from file.
                    ...
                    ############## for R to L:#########################
                    #
                    startfreq = 86.72E6 # need to -spacing for L waveforms, don't for R waveforms (0.64 lambda spacing, 101.44 center frequency)
            ...
            for sweep_num in np.arange(ntraps - 2) + 1:
                name_temp = 'sweep46tweezers_{}.h5'.format(sweep_num)
                filename = Path(folder_name, name_temp)
                # If we have already computed the Waveforms...
                # if os.access(filename + '.h5', os.F_OK):  # ...retrieve the Waveforms from file.
                if os.access(filename, os.F_OK):  # ...retrieve the Waveforms from file.
                    ...
                    startfreq = 86.72E6 - spacing  # need to -spacing for L waveforms, don't for R waveforms
                    f_list = [startfreq + j * spacing for j in range(ntraps)]
                    for i in range(ntraps):
                        if i <= sweep_num:
                            freq_A.append(f_list[i])
