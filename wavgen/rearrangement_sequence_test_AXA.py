    ...
    def on_created(self, event):
        ...
        if path != self.last_created:
            ...
            if 0 < atom_count:
                ...
                llSegment = 2 * num_tweezers - 1 + self.drop_counter  # the drop waveform
                llLoop = int(3 * 0.001 * SAMP_FREQ / wf_list[llSegment].SampleLength)  # pattern repeated once
                ...
                lStep = 1  # current step is step 1
                llSegment = 2*num_tweezers-1 + self.drop_counter #2*num_tweezers - 2   # static
