            ...
            if 0 < atom_count:
                ...
                if len(segment_queue_L) > 0:
                    ...
                    if multi_trig==True and seq=='ANA':
                        ...
                        # for loop_num in range(num_cicero_loops-2):
                        #     lStep = 2 * num_tweezers + 22 + loop_num
                        #     llSegment = 2*num_tweezers + loop_num # the shifted (node) waveform
                        #     llLoop = 1 #int(0.3*SAMP_FREQ/wf_list[-1].SampleLength)
                        #     llNext = 2 * num_tweezers + 22 + loop_num + 1 # 0 # 2 * num_tweezers + 100  # next step is 0
                        #     llCondition = SPCSEQ_ENDLOOPONTRIG # SPCSEQ_ENDLOOPALWAYS  # unconditionally leave current step
                        #     llValue = (llCondition << 32) | (llLoop << 32) | (llNext << 16) | (llSegment)
                        #     spcm_dwSetParam_i64(hCard, SPC_SEQMODE_STEPMEM0 + lStep, int64(llValue))
...
if __name__ == '__main__':
    ...
    if multi_trig:
        if seq == 'ANA':
            ...
            multi_trig_list = ['sweep_forward_4Lo16.h5', 'static_shifted_4Lo16.h5',
                               'sweep_back_4Lo16.h5', 'static.h5']  # integer ANA
