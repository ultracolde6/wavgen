...
class TestEventHandler(PatternMatchingEventHandler):
    ...
    def on_created(self, event):
        ...
        if path != self.last_created:
            ...
            ########################## new for resorting to the center ######################
            boundary = 20 #int(ntraps/2)
            ...
            if 0 < atom_count:
                ...
                if len(segment_queue_R) > 0 and len(segment_queue_L) > 0:
                    print('right resorting 1')
                    ...
                    print('right resorting 2')
                    for k in range(1, len(segment_queue_R)):
                        lStep = k + 1  # current step is step k+1 (+1 because step0 is the static config)
                        llSegment = segment_queue_R[k]  # associated data memory segment
                        llLoop = 1  # pattern repeated once
                        llNext = k + 2  # next step is the next sweep
                    ...
                    lStep = 1
                    llSegment = len(segment_list)-1 # + self.i_counter # the drop waveform
                    llLoop = 1  # pattern repeated once
                    llNext = 0  # next step is 0
                    llCondition = SPCSEQ_ENDLOOPALWAYS  # unconditionally leave current step
                    llValue = (llCondition << 32) | (llLoop << 32) | (llNext << 16) | (llSegment)
                ...
                lStep = 1  # current step is step 1
                llSegment = num_tweezers - 1  # I don't think there's any significance to which segment we choose here
                llLoop = 1  # pattern repeated once
...
if __name__ == '__main__':
    path_folder = 'waveforms_160_16Twz_5lambda_v2' #'waveforms_160_16Twz_1MHz'
    ntraps = 16  # this is the num of tweezers we want
    cycle_list = ['static.h5']
    ...
    # filename_list = ['sweep_1.h5', 'sweep_2.h5', 'sweep_3.h5', 'sweep_4.h5', 'sweep_5.h5', 'sweep_6.h5', 'sweep_7.h5',
    #                  'sweep_8.h5', 'sweep_9.h5', 'sweep_10.h5', 'sweep_11.h5', 'sweep_12.h5', 'sweep_13.h5',
    #                  'sweep_14.h5', 'sweep_15.h5']
  ...
  ################################################
    ...
    max_segments = len(segment_list)  # num_tweezers + N_cycle
...
########################################################################################################################
    ...
    llSegment = segment_list[-1]  # associated data memory segment
    llLoop = 1  # pattern repeated once
