            ...
            if 0 < atom_count:
                ...
                if len(segment_queue_L) > 0:
                    ...
                    if len(segment_queue_R) > 0:
                        ...
                        # llLoop = int(25*4003200/wf_list[-1].SampleLength)  # pattern repeated
                        llLoop = int(1*0.001*SAMP_FREQ/wf_list[-1].SampleLength)  # int(16*6400000/wf_list[-1].SampleLength)  # pattern repeated
                        llNext = 0  # 2 * num_tweezers + 22  # 0  # next step is 0
                        # llCondition = SPCSEQ_ENDLOOPALWAYS  # unconditionally leave current step
                        # llValue = (llCondition << 32) | (llLoop << 32) | (llNext << 16) | (llSegment)
                        # spcm_dwSetParam_i64(hCard, SPC_SEQMODE_STEPMEM0 + lStep, int64(llValue))
                    llCondition = SPCSEQ_ENDLOOPALWAYS  # unconditionally leave current step
                    ...
                    if len(segment_queue_R) > 0:
                        ...
                        # lStep = 2 * num_tweezers + 20
                        # llSegment = 2*num_tweezers-2 # static
                        # llLoop = int(103.2e6/wf_list[2*num_tweezers-2].SampleLength)  # int(25*4003200/wf_list[-1].SampleLength)  # pattern repeated
                        # print(f'llLoop = {llLoop}')
                        # llNext = 2 * num_tweezers + 22  # 0  # next step is 0
                        # llCondition = SPCSEQ_ENDLOOPALWAYS  # unconditionally leave current step
                        # llValue = (llCondition << 32) | (llLoop << 32) | (llNext << 16) | (llSegment)
                        # spcm_dwSetParam_i64(hCard, SPC_SEQMODE_STEPMEM0 + lStep, int64(llValue))
                        # print('drop step', int(2*0.001*SAMP_FREQ/wf_list[2*num_tweezers -1 + self.i_counter].SampleLength))
                        lStep = 2*num_tweezers + 21
                        llSegment = 2*num_tweezers -1 + self.i_counter # the drop waveform
                        # llLoop = int(25*4003200/wf_list[-1].SampleLength)  # pattern repeated
                        llLoop = int(5*0.001*SAMP_FREQ/wf_list[2*num_tweezers -1+ self.i_counter].SampleLength)  # pattern repeated once
                        ...
                        # # llLoop = int(25*4003200/wf_list[-1].SampleLength)  # pattern repeated
                        # llLoop = 1  # int(16*6400000/wf_list[-1].SampleLength)  # pattern repeated once
                        # llNext = 2*num_tweezers + 23 #0  # next step is 0
                    ...
                    if multi_trig == True:
                        ...
                        # lStep = 2 * num_tweezers + 22 + num_cicero_loops-1
                        # llSegment = 2 * num_tweezers - 2 #segment_list[-1]  # the shifted waveform
                        # llLoop = 1
                        # llNext = 0  # next step is 0
                        # llCondition = SPCSEQ_ENDLOOPALWAYS  # unconditionally leave current step
                        # llValue = (llCondition << 32) | (llLoop << 32) | (llNext << 16) | (llSegment)
                    ...
                    llSegment = 2 * num_tweezers - 1 + self.i_counter # the drop waveform
                    # llLoop = int(25*4003200/wf_list[-1].SampleLength)  # pattern repeated
                    llLoop = int(0.1*0.001*SAMP_FREQ/wf_list[-1].SampleLength)  # pattern repeated
                    llNext = 0 #2 * num_tweezers + 22  # 0  # next step is 0
                    llCondition = SPCSEQ_ENDLOOPALWAYS  # unconditionally leave current step
                    llValue = (llCondition << 32) | (llLoop << 32) | (llNext << 16) | (llSegment)
                    spcm_dwSetParam_i64(hCard, SPC_SEQMODE_STEPMEM0 + lStep, int64(llValue))
                    ...
                    # lStep = 2 * num_tweezers + 22
                    # llSegment = 2 * num_tweezers  # + self.i_counter # the drop waveform
                    # llLoop = 1  # int(25*4003200/wf_list[-1].SampleLength)  # pattern repeated
                    # llNext = 0  # next step is 0
                    # llCondition = SPCSEQ_ENDLOOPALWAYS  # unconditionally leave current step
                    # llValue = (llCondition << 32) | (llLoop << 32) | (llNext << 16) | (llSegment)
                    # spcm_dwSetParam_i64(hCard, SPC_SEQMODE_STEPMEM0 + lStep, int64(llValue))
...
if __name__ == '__main__':
    ...
    if multi_trig:
        ...
        spacing = 0.8
        startfreq = 88
        # multi_trig_list = ['sweep_5to5,5lambda_shifted_Lo4.h5', 'static_5,5lambda_shifted_Lo4.h5', 'sweep_LtoR_quarter_lambda_2nd.h5',
        #  'static_neg.h5', 'sweep_RtoL_quarter_lambda_3nd.h5']
        multi_trig_list = ['sweep_5to5,5lambda_shifted_Lo4.h5',
                           'static_spock_node.h5',
                           'sweep_5,5to5,5_node_to_spock_1Lo64.h5',
         'static_spock_1Lo64.h5', 'sweep_5,5to5lambda_spock_1Lo64.h5']
        # multi_trig_list = ['sweep_LtoR_quarter_lambda.h5', 'shift_quarter_lambda.h5', 'sweep_RtoL_quarter_lambda.h5',
        #  'static.h5', 'static.h5']
        # multi_trig_list = ['sweep_5to5,5lambda_shifted_Lo4.h5', 'static_5,5lambda_shifted_Lo4.h5', 'sweep_Lo4_5,5lambda.h5', 'static_5,5lambda.h5', 'sweep_5,5to5lambda.h5']
    else:
        path_folder = 'waveforms_160_40Twz_5lambda_v2'
        spacing = 0.8
        startfreq = 88
        multi_trig_list = []
    cycle_list = ['drop_20.h5']
