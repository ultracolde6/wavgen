        ...
        if path != self.last_created:
            ...
            if 0 < atom_count:
                ...
                if len(segment_queue_L) > 0:
                    ...
                    llSegment = int(len(wf_list)-3) # 2 * num_tweezers  # sweep to 5.5lambda shifted by Lo4
                    llLoop = 1
                    ...
                    llSegment = int(len(wf_list)-2) # 2 * num_tweezers + 1  # 5.5lambda shifted by Lo4
                    llLoop = 1
...
if __name__ == '__main__':
    ...
    #                    'sweep_5,5to5lambda_spock_1Lo32.h5']

    AXA_list = [['sweep_5to5,5lambda_shifted_Lo4.h5', 'static_spock_node.h5', 'sweep_5,5to5lambda_spock_1Lo32.h5'],
                ['sweep_5to5,5lambda_shifted_Lo4.h5', 'static_spock_node.h5', 'sweep_5,5to5lambda_spock_1Lo32.h5'],
                ['sweep_5to5,5lambda_shifted_Lo4.h5', 'static_spock_node.h5', 'sweep_5,5to5lambda_spock_1Lo32.h5'],
                ['sweep_5to5,5lambda_shifted_Lo4.h5', 'static_spock_node.h5', 'sweep_5,5to5lambda_spock_1Lo32.h5']]

    drop_list = ['DROP/drop_20.h5']
    ...
    #################### include dwell waveform ########################
    # this is static.h5
    #################### include sorting waveforms ########################
    sort_list_L = [f'SORT/sweep_{num}.h5' for num in range(1, ntraps)]
    sort_list_R = [f'SORT/sweep_{num}R.h5' for num in range(1, ntraps)]
    sort_list = np.concatenate((filename_list_L, filename_list_R))
    # print(filename_list)
    wf_list = []
    filename_list = sort_list
    ...
    filename_list.append(drop_list)
    for filename in drop_list:
        wf_list.append(utilities.from_file_simple(Path(path_folder, filename), 'A'))

    ####################### include static waveforms #########################
    filename_list.append(static_list)
    for filename in static_list:
        wf_list.append(utilities.from_file_simple(Path(path_folder, filename), 'A'))

    ####################### include AXA sweep waveforms #######################
    filename_list.append(sweep_list)
    for filename in sweep_list:
        wf_list.append(utilities.from_file_simple(Path(path_folder, filename), 'A'))

    num_wf = len(wf_list)
    segment_list = range(num_wf)
